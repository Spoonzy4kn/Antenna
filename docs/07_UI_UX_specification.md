...
It describes how users move through the application, how the main screens are structured, and how the interface should behave.

This specification is expected to evolve as the application design becomes more detailed.
...
These are not primary navigation items because they are not part of the regular browsing flow. They belong inside Settings to keep the main interface clean.
...
## 6. Global screen states

Every major screen should define behaviour for the following states:

| State | Meaning | Expected UX |
| --- | --- | --- |
| Idle | Screen has not yet requested data. | Show stable layout or first-use prompt. |
| Loading | Data is being requested. | Show non-blocking progress indicator. |
| Loaded | Data loaded successfully. | Show content. |
| Empty | Request succeeded but no content exists. | Explain the empty state and next possible action. |
| Error | Request failed. | Show human-readable message and retry where practical. |
| Offline | Device appears offline. | Explain network issue and allow retry. |
| Restricted | Content exists but is inaccessible. | Explain restriction where practical. |
| Unsupported | Content type cannot be displayed. | Explain limitation rather than failing silently. |

## 7. Home

Home is the primary feed screen.

Home shall show posts from communities saved in My Subs where supported.

Home is not intended to be a dashboard, welcome page, promotional page, or algorithmic recommendation surface.
...
```text
No communities saved yet.
Add communities through Search, Explore, or My Subs to build your local feed.
```
...
- Searching users where supported.
...
### 11.4 My Subs empty state

Suggested copy:

```text
No saved communities.
Add communities to My Subs to build a local feed without signing in.
```
...
## 14. Sort and filter distinction
...
- Consistent sort and filter behaviour.
- Clear distinction between anonymous and account-required functionality.
